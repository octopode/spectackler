...
def log_data(logfile, deques):
	...
	data_dict = {"clock": '', "watch": '', "inj": False}
	with open(logfile, 'w', buffering=1) as handout:
		...
		while True:
			...
			if data_dict != data_prev:
				# mark time and injection status
				data_dict.update({
...
# data to where?
logfile = "/Applications/spectackler/fusiondata.dat"

reptime_pedal = 3 # in s
# low pressure
press_lo = 0.7
# high pressure
press_hi = float(input("enter target pressure in bar "))

# pump should be in PGa mode
pump.remote()
#pump.digital(0,1) # if cold

# establish initial pressure
input("press ENTER to start data collection")
#pump.press_set(press_lo)
#pump.run()
#pump.digital(0,1) # if cold
...
input("press ENTER to mark injection")
print('\a') # ding!
inj = True
time.sleep(reptime_pedal)

# start sweep
input("press ENTER to go to {} bar".format(press_hi))
pump_free.clear()
pump.press_set(press_hi)
pump.run()
pump_free.set()
time.sleep(reptime_pedal)

# start sweep
input("press ENTER to go to {} bar".format(press_lo))
pump_free.clear()
pump.press_set(press_lo)
pump.run()
pump_free.set()
