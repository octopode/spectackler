...
pat_data <- "*JWL0032_laurdan*" # filename pattern
...
# live plotting loop
refresh = 5 # (seconds)
img_out = "~/Documents/MBARI/20210118_JWL0027_laurdan.pdf"
...
while(TRUE){
  ...
  # plot GP
  plot_gp <- contourplot(gpdata, lims_t = range(rawdata$T_act), lims_p = range(rawdata$P_act)) + ggtitle("GP vs. Pressure vs. Temperature")
...
contourplot <- function(gpdata, nox=100, noy=100, nbins=9, gpmin=-0.1, gpmax=0.4, lims_t=c(3,28), lims_p=c(0,500)){
  gpdata_clean <- gpdata %>%
  ...
  gpdata_clean %>%
    ...
    geom_contour_filled(aes(fill=stat(level_mid)), bins=nbins) +
