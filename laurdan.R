...
pat_data <- "20200722_DOPC_9x9" # filename pattern

# this plot function can be called in a loop for live monitoring!
plot_landscape <- function(dir, pat){
  ...
  plot_pt <- alldata %>% 
    select(T_act, P_act) %>% 
    ggplot(aes(x = T_act, y = P_act)) +
    geom_point(color = "black", alpha = 0.017) +
    theme_pubr() +
  ...
  plot_gp <- gpdata %>%
    ...
    ggplot(aes(x=P_act, y=T_act, z=gp, fill=gp)) +
    geom_raster() +
  ...
  grid.arrange(plot_pt, plot_gp, ncol=1)
}
...
while(TRUE){
  time_start <- Sys.time()
  dev.off() # clear the prev plot
  plot_landscape(dir_data, pat_data)
  Sys.sleep(refresh - (Sys.time() - time_start))
}
