...
The landscape is provided as a TSV via stdin.
If the fluorescence intensity gets below a certain threshold,
the script will pause and send an iMessage so the operator can come zap the sample.
v1.3 (c) JRW 2021 - jwinnikoff@mbari.org
...
def zapdat(pump, pump_free, email):
    "send notification, bring sample to the surface, and wait"
    cmd_mail = "mail"
    # os.system(cmd_mail)
    pump_free.clear()
    press = pump.press_get()
    while not pump.press_set(0.7): pass
    pump_free.set() # resume data logging
    # make sure stdin is the command line
    sys.stdin = open('/dev/tty')
    input("sonicate the cuvette, replace and press RET to resume")
    pump_free.clear()
    # return to prev pressure
    while not pump.press_set(press): pass
...
def parse_args(argv):
    ...
    parser.add_argument('-e', "--eqls", help="dict of tuples of (min, max) equilibration times for each variable", type=eval, default='{"T_set":(60,1500), "P_set":(60,1500)}') # reset to 60,1500
    parser.add_argument('-t', "--tols", help="dict of max change over equilibration time for each variable", type=eval, default='{"T_act":0.5, "P_act":0.5}')
    ...
    parser.add_argument("--alert", help="Low fluor signal at which to pause and issue email alert", type=float, default=3.0)
    parser.add_argument("--email", help="Email address for alerts", type=str, default="8057088415@mms.att.net")
...
def main(args, stdin, stdout, stderr):
    ...
    with open(args['file_log'], 'x') as hand_log:
        ...
        try:
            ...
            amcu = auxmcu.AuxMCU(port=args['port_amcu'], init=False)
            ...
            amcu.lamp(True)
            #amcu.wheels_init() # don't move them! should be wide open
                    ...
                    if all(in_range.values()):
                        ...
                        if ("T_set" not in vars_wait) or ((time.time() - args["shut_sit"]) >= time_open):
                            ...
                            if (readings > args["n_read"]):
                                # if more than half the readings are too low, pause and reset
                                if sum([(reading <= args["alert"]) for reading in trails['intensity'].iloc[-1*args["n_read"]:-1]]) >= (args["n_read"]/2):
                                    zapdat(pump, pump_free, args["email"])
                                    readings = 0
                                else:
