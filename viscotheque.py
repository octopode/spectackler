...
# as of 20220103, NeslabRTE does not work under py3. Fix is #TODO.
...
def parse_args(argv):
    ...
    parser.add_argument('-r', "--rtd_cal", help="Internal RTD cal slope and intercept (waterbath to sample temp at equilibrium)", type=eval, default='(1.341635, -5.255324)')
...
def main(args, stdin, stdout, stderr):
    ...
    with open(args['file_log'], 'x') as hand_log:
        ...
        try:
            ...
            print("fluorospectrometer     ", end='', file=stderr)
            spec = rf5301.RF5301(port=args['port_spec'])
            print("√", file=stderr)
            print("aux microcontroller    ", end='', file=stderr)
            ...
            print("√", file=stderr)
            print("pressure controller    ", end='', file=stderr)
            ...
            # initialize the AMCU
            print("auxiliary", end='', file=stderr)
            amcu.lamp(True)
            print(' √', file=stderr)
            ...
            for state_num in range(states.shape[0]):
                ...
                # init the data dict. Persistent at start of each state.
                data_dict = {
                ...
                if not state_num:
                    hand_log.write('\t'.join(sorted(set(state_curr.keys() + data_dict.keys()))) + '\n')
                ...
                # set temp
                bath_free.clear()
                while not isclose(bath.temp_set(), bath.cal_int.act2ref(state_curr['T_set'])):
                ...
                #    if spec.wl_ex(state_curr['wl_ex']): print('√', file=stderr)
                #    
                ...
                #    if spec.wl_ex(state_curr['wl_em']): print('√', file=stderr)
                ...
                if not (
                    ...
                    if (state_curr['wl_ex'] == 350) and (state_curr['wl_em'] == 428):
                        ...
                        if spec.wl_set_dph(): print('√', file=stderr)
                    elif (state_curr['wl_ex'] == 340) and (state_curr['wl_em'] == 440):
                        print("setting wavelengths to 340/440", file=stderr, end=' ')
                        if spec.wl_set_340_440(): print('√', file=stderr)
                    elif (state_curr['wl_ex'] == 340) and (state_curr['wl_em'] == 490):
                        print("setting wavelengths to 340/490", file=stderr, end=' ')
                        if spec.wl_set_340_490(): print('√', file=stderr)
                    elif (state_curr['wl_ex'] == 410) and (state_curr['wl_em'] == 440):
                        print("setting wavelengths to 410/440", file=stderr, end=' ')
                        if spec.wl_set_410_440(): print('√', file=stderr)
                    elif (state_curr['wl_ex'] == 410) and (state_curr['wl_em'] == 490):
                        print("setting wavelengths to 410/490", file=stderr, end=' ')
                        if spec.wl_set_410_490(): print('√', file=stderr)
                        
                    # slits
                    if state_curr['slit_ex'] != data_dict['slit_ex']:
                        print("setting ex slit to {} nm".format(state_curr['slit_ex']), file=stderr, end=' ')
                        if spec.slit_ex(state_curr['slit_ex']): print('√', file=stderr)
                    if state_curr['slit_em'] != data_dict['slit_em']:
                        print("setting em slit to {} nm".format(state_curr['slit_em']), file=stderr, end=' ')
                        if spec.slit_em(state_curr['slit_em']): print('√', file=stderr)
                    #spec_free.set()
                ...
                while True:
                    ...
                    if data_dict["intensity"] != data_prev["intensity"]:
                    	...
                    	if (data_dict["watch"] - time_state) < data_dict["time"]:
                    	    print("waiting {}/{} s".format(round(time.time()-time_state), data_dict["time"]), file=sys.stderr, end='\r')
                    	    waited = True
                    	else:
                    	    ...
                    	    if (readings+1) >= data_dict["n_read"]:
